...
use graphics::{NGEncoder, NGFactory, NGResources, NGTexture, OutColor, OutDepth, RenderType,
               Shaders, pipeline_spritesheet, pipeline_text};
    ...
    ID: Send + Eq,
{
    ...
    pub fn add_render_text(
        &mut self,
        factory: &mut NGFactory,
        packet: &pipeline_text::Packet,
        character: char,
    ) {
        let shaders = {
        ...
        let shader_set = factory
            .create_shader_set(shaders.get_vertex_shader(), shaders.get_fragment_shader())
            .unwrap_or_else(|err| panic!("Create Shader Set Error: {:?}", err));

        let program = factory.create_program(&shader_set).unwrap_or_else(|err| {
            panic!("Create Program Error: {:?}", err)
        });
        ...
        let pso = factory
            .create_pipeline_from_program(
                ...
                pipeline_text::pipe::new(),
            )
            .unwrap_or_else(|err| {
                panic!("Create Pipeline from Program Error: {:?}", err)
            });
        ...
        let mut bundles =
            Arc::get_mut(&mut self.bundles_text).unwrap_or_else(|| panic!("Arc Shit"));
    ...
    fn get_character_index(&self, character: char) -> usize {
        *self.character_map.get(&character).unwrap_or_else(|| {
            panic!("Character not in Character Map")
        })
    }

    pub fn add_render_spritesheet(
        &mut self,
        factory: &mut NGFactory,
        packet: &pipeline_spritesheet::Packet,
        texture: NGTexture,
    ) -> usize {
        let shaders = {
            if self.shaders.contains_key(&RenderType::Spritesheet) {
                ...
                self.shaders.insert(
                    ...
                    pipeline_spritesheet::make_shaders(),
                );
                &self.shaders[&RenderType::Spritesheet]
            }
        };
        ...
        let mut bundles =
            Arc::get_mut(&mut self.bundles_spritesheet).unwrap_or_else(|| panic!("Arc Shit"));
    ...
    fn render(&mut self, arg: &RunArg, mut encoder: NGEncoder, encoder_id: ID) {
        ...
        let (mut transforms, mut cameras, mut render_datas_spritesheet, mut render_datas_text) =
            arg.fetch(|w| {
                (
                    w.write::<Transform>(),
                    w.write::<Camera>(),
                    w.write::<RenderDataSpritesheet>(),
                    w.write::<RenderDataText>(),
                )
            });
        ...
        //render spritesheets
        {
            ...
            for (transform, render_data_spritesheet) in
                (&mut transforms, &mut render_datas_spritesheet).join()
            {
                ...
                let model_data =
                    pipeline_spritesheet::ModelData { model: transform.get_model().into() };
                ...
                datas.push((
                    render_data_spritesheet.get_render_id_num(),
                    render_data_spritesheet.get_layer(),
                    texture_data,
                    camera_data,
                    model_data,
                ));
            ...
            for data in datas {
                let b = self.bundles_spritesheet.get(data.0).unwrap_or_else(|| {
                    panic!("No Bundle found")
                });
        ...
        //render text
        {
            ...
            for (transform, render_data_text) in (&mut transforms, &mut render_datas_text).join() {
                ...
                let model_data = pipeline_text::ModelData { model: transform.get_model().into() };

                let color = pipeline_text::Color { color: render_data_text.get_color().clone() };

                let mut i = 0;

                for character in render_data_text.get_text().chars() {
                    datas.push((
                        self.get_character_index(character),
            ...
            for data in datas {
                let b = self.bundles_text.get(data.0).unwrap_or_else(
                    || panic!("No Bundle found"),
                );
        ...
        self.channel_main.send(MainFromRender::Encoder(
            encoder,
            encoder_id,
        ));
    }
