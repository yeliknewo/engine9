...
pub mod glutin {
    ...
    pub fn handle_events<ID>(
        gfx_window: &mut GfxWindow<GlutinWindow, GlutinExtras, NGDevice, NGFactory, NGResources>,
        front_event_clump: &mut FrontEventClump<ID>,
    ) -> bool
    where
        ID: Send + Eq,
    {
            ...
            match event {
                ...
                Event::MouseMoved(x, y) => {
                    front_event_clump
                        ...
                        .send(MainToControl::MouseMoved(x, y));
                }
                Event::MouseInput(state, button) => {
                    ...
                    if let Some(new_button) = new_button_opt {
                        front_event_clump
                            .get_mut_control()
                            .unwrap_or_else(|| panic!("Control was none"))
                            .send(MainToControl::MouseButton(new_state, new_button));
                    }
                }
