...
impl System<f64> for SystemControl {
    fn run(&mut self, arg: RunArg, _delta_time: f64) {
        let (cameras, transforms, mut buttons) = arg.fetch(|w| {
            (
                w.read::<Camera>(),
                w.read::<Transform>(),
                w.write::<Button>(),
            ...
            match event {
                MainToControl::MouseButton(state, mouse_button) => {
                    ...
                    for (transform, mut button) in (&transforms, &mut buttons).join() {
                        let bot_left = transform.get_model() *
                            Vector4::new(
                                button.get_bot_left().x,
                                button.get_bot_left().y,
                                1.0,
                                1.0,
                            );

                        let top_right = transform.get_model() *
                            Vector4::new(
                                button.get_top_right().x,
                                button.get_top_right().y,
                                1.0,
                                1.0,
                            );

                        if mouse_in_world.x >= bot_left.x && mouse_in_world.x <= top_right.x &&
                            mouse_in_world.y >= bot_left.y &&
                            mouse_in_world.y <= top_right.y
                        {
