...
impl System<f64> for SystemMainGameCity {
    fn run(&mut self, arg: RunArg, _delta_time: f64) {
        let (mut cities, maps, tiles, mut render_datas_spritesheet) = arg.fetch(|w| {
            (
                w.write::<City>(),
                w.read::<Map>(),
                w.read::<Tile>(),
        ...
        if let Some(entity_map) = self.entity_opt_map {
            if let Some(map) = maps.get(entity_map) {
                for (mut city, mut render_datas_spritesheet) in
                    (&mut cities, &mut render_datas_spritesheet).join()
                {
                    ...
                    for y in -city_range..city_range {
                        for x in -city_range..city_range {
                            ...
                            if let Some(tile_entity) = map.get_tile(&tile_pos) {
                                let tile = tiles.get(*tile_entity).unwrap_or_else(|| {
                                    panic!("Tile Entity wasn't in Tiles")
                                });
                    ...
                    render_datas_spritesheet.set_spritesheet_rect(
                        art::main::green::ARRAY[city.get_size() &
                                                    art::main::green::ARRAY
                                                        .len()],
                    );
                }
            } else {
