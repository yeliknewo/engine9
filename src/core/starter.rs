...
pub fn start() {
    ...
    let game_handle = {
        ...
        {
            let mut render_event_core = front_event_clump.get_mut_render().unwrap_or_else(|| {
                panic!("Render was none")
            });

            render_event_core.send(MainToRender::Encoder(
                gfx_window.get_mut_factory().create_command_buffer().into(),
                0,
            ));
            render_event_core.send(MainToRender::Encoder(
                gfx_window.get_mut_factory().create_command_buffer().into(),
        ...
        let mut renderer = SystemRender::new(
            back_event_clump.take_main_x_render().unwrap_or_else(|| {
                panic!("Main X Render was none")
            }),
            out_color,
            out_depth,
        );
        ...
        let assets = Search::ParentsThenKids(2, 2)
            .for_folder("assets")
            .unwrap_or_else(|err| {
                panic!("Did you forget to make an assets folder? Err: {:?}", err)
            });
    ...
    'main: loop {
        if let Some(event) = front_event_clump
            .get_mut_render()
            ...
            match event {
                MainFromRender::Encoder(mut encoder, encoder_id) => {
                    ...
                    front_event_clump
                        .get_mut_render()
                        .unwrap_or_else(|| panic!("Render was none"))
                        .send(MainToRender::Encoder(encoder, encoder_id));
    ...
    game_handle.join().unwrap_or_else(
        |err| panic!("Join Error: {:?}", err),
    );
}
