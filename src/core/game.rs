...
use components::{Button, Camera, City, Map, RenderDataSpritesheet, RenderDataText, Scene, Tile,
                 Transform};
...
use events::{GameFromMainGameCity, GameFromMainGameMap, GameFromMainGamePauseMenu,
             ...
             GameToMainMenu, MainFromGame, MainToGame};
use specs::{Entity, Planner, System, SystemInfo, World};
use std::collections::HashMap;
use systems::{SystemControl, SystemFpsCounter, SystemMainGameCity, SystemMainGameMap,
              SystemMainGamePauseMenu, SystemMainMenu, SystemRender};
...
impl Game {
    pub fn new<ID>(
        render_system: SystemRender<u64>,
        main_render: usize,
        mut back_event_clump: BackEventClump<ID>,
        ...
        ID: 'static + Eq + Send,
    {
        ...
        planner.add_system(
            SystemControl::new(
                back_event_clump.take_main_x_control().unwrap_or_else(|| {
                    panic!("Main X Control was none")
                }),
                screen_size,
            ),
            SYSTEM_CONTROL.0,
        ...
        planner.add_system(
            SystemFpsCounter::new(WARN_FPS),
            SYSTEM_FPS_COUNTER.0,
            SYSTEM_FPS_COUNTER.1,
        );

        let mut inactive_systems: HashMap<SystemIdent, Box<System<f64>>> = HashMap::new();

        inactive_systems.insert(
            SYSTEM_MAIN_GAME_PAUSE_MENU,
            Box::new(SystemMainGamePauseMenu::new(
                back_event_clump
                    .take_back_game_x_main_game_pause_menu()
                    .unwrap_or_else(|| panic!("Back Game X Main game Pause Menu was none")),
            )),
        );
        inactive_systems.insert(
            SYSTEM_MAIN_MENU,
            Box::new(SystemMainMenu::new(
                back_event_clump
                    .take_back_game_x_main_menu()
                    .unwrap_or_else(|| panic!("Back Game X Main Menu was none")),
            )),
        );
        inactive_systems.insert(
            SYSTEM_MAIN_GAME_CITY,
            Box::new(SystemMainGameCity::new(
                back_event_clump
                    .take_back_game_x_main_game_city()
                    .unwrap_or_else(|| panic!("Back Game X Main Game City was none")),
            )),
        );
        inactive_systems.insert(
            SYSTEM_MAIN_GAME_MAP,
            Box::new(SystemMainGameMap::new(
                back_event_clump
                    .take_back_game_x_main_game_map()
                    .unwrap_or_else(|| panic!("Back Game X Main Game Map was none")),
            )),
        );

        let mut game = Game {
            last_time: precise_time_ns(),
            channel_main: back_event_clump.take_main_x_game().unwrap_or_else(|| {
                panic!("Main X Game was none")
            }),
            channel_main_menu: back_event_clump
                .take_front_game_x_main_menu()
                .unwrap_or_else(|| panic!("Front Game X Main Menu was none")),
            channel_main_game_pause_menu:
                back_event_clump
                    .take_front_game_x_main_game_pause_menu()
                    .unwrap_or_else(|| panic!("Front Game X Main Game Pause Menu was none")),
            channel_main_game_map:
                back_event_clump
                    .take_front_game_x_main_game_map()
                    .unwrap_or_else(|| panic!("Front Game X Main Game Map was none")),
            channel_main_game_city:
                back_event_clump
                    .take_front_game_x_main_game_city()
                    .unwrap_or_else(|| panic!("Front Game X Main Game City was none")),
    ...
    fn create_main_game_scene(&mut self) {
        ...
        let map = self.planner
            .mut_world()
            .create_now()
            .with(Map::new(min, max))
            .build();

        self.add_to_scene(map);

        self.channel_main_game_map.send(
            GameToMainGameMap::SetEntityMap(
                map,
            ),
        );
        self.channel_main_game_city.send(
            GameToMainGameCity::SetEntityMap(map),
        );
        ...
        for y in min.y..max.y {
            for x in min.x..max.x {
                ...
                let tile = self.planner
                    ...
                    .with(RenderDataSpritesheet::new(
                        self.main_render,
                        art::layers::PLAYER,
                        *art::main::DEFAULT_TINT,
                        art::main::yellow::BLANK,
                        art::main::SIZE,
                    ))
                ...
                self.planner
                    ...
                    .write_w_comp_id::<Map>(())
                    .get_mut(map)
                    .unwrap_or_else(|| panic!("Unable to Get Mut Map"))
                    .set_tile(Vector2::new(x, y), tile);
            }
        }
        ...
        self.channel_main_game_city.send(
            GameToMainGameCity::SetEntityCity(city),
        );
        ...
        let exit_button = self.planner
            ...
            .with(Transform::new(
                Vector3::new(
                    -0.5 * 1.2 + exit_x_offset,
                    -2.0 * 1.2 + exit_y_offset,
                    0.0,
                ),
                Euler::new(Rad(0.0), Rad(0.0), Rad(0.0)),
                Vector3::new(5.0 * 1.2, 5.0 * 1.2, 1.0),
            ))
            .with(RenderDataSpritesheet::new(
                self.main_render,
                art::layers::GUI,
        ...
        let exit_button_text = self.planner
            ...
            .create_now()
            .with(Transform::new(
                Vector3::new(exit_x_offset, exit_y_offset, 0.0),
                Euler::new(Rad(0.0), Rad(0.0), Rad(0.0)),
                Vector3::new(1.0, 1.0, 1.0),
            ))
            .with(RenderDataText::new(
                art::layers::GUI,
                "exit".into(),
                art::colors::WHITE,
                1.2,
        ...
        self.channel_main_game_pause_menu.send(
            GameToMainGamePauseMenu::SetEntitiesExitButton(exit_button, exit_button_text),
        );
    }
    ...
    fn create_main_menu_scene(&mut self) {
        ...
        let play_button = self.planner
            ...
            .create_now()
            .with(Transform::new(
                Vector3::new(-0.5 * 1.2, -2.0 * 1.2, 0.0),
                Euler::new(Rad(0.0), Rad(0.0), Rad(0.0)),
                Vector3::new(5.0 * 1.2, 5.0 * 1.2, 1.0),
            ))
            .with(RenderDataSpritesheet::new(
        ...
        self.channel_main_menu.send(
            GameToMainMenu::SetEntitiesPlayButton(play_button, play_button_text),
        );
    }
    ...
    fn deactivate_system(&mut self, system_ident: SystemIdent) {
        for i in 0..self.planner.systems.len() {
            if self.planner.systems[i].name == system_ident.0 {
                self.inactive_systems.insert(
                    system_ident,
                    self.planner.systems.remove(i).object,
                );
                break;
            }
        }
